...
class M_items extends CI_Model{
    ...
    public function get_allItemsforJSON()
    {
        ...
        $this->db->select('B.item_id,B.category_id, B.name,B.service,A.barcode,A.inventory_acc_code,
        A.size_id,A.color_id,A.item_type,A.unit_id,A.quantity,A.avg_cost,A.cost_price,A.unit_price,A.re_stock_level,
        A.inventory_acc_code,A.wip_acc_code,A.landed_cost,A.local_currency,A.quoted_qty,A.hs_code,
        U.name as unit_name,S.name AS supplier,S.dtl_cost, CAT.name as category');
        $this->db->join('pos_items AS B','A.item_id = B.item_id','left');
        ...
        $this->db->join('pos_categories as CAT','B.category_id = CAT.id','left');
        //$this->db->join('pos_shipping_terms as ST','A.shipping_terms = ST.id','left');
        $this->db->join('pos_supplier as S','A.supplier_id = S.id','left');
    ...
    public function get_allItems($supplier_id = '')
    {
        ...
        $this->db->select('A.item_id,A.service,A.category_id, A.name,B.inventory_acc_code,B.barcode,B.size_id,B.unit_id,
        B.color_id,B.quantity,B.cost_price, B.avg_cost,B.unit_price,B.re_stock_level,
        B.inventory_acc_code,B.wip_acc_code');
        $this->db->join('pos_items_detail AS B','A.item_id = B.item_id','left');
    ...
    {
        //$this->db->order_by('A.item_id','desc');
        $this->db->select('B.item_id,A.service,A.category_id, A.name');
        //B.color_id,B.quantity,B.cost_price, B.avg_cost,B.unit_price');
        $this->db->join('pos_items AS A','A.item_id = B.item_id');
        //$this->db->join('pos_taxes AS T','T.id = B.tax_id','left');
        //$this->db->join('pos_sizes as C','B.size_id = C.id','left');
        
        $query = $this->db->get_where('pos_items_detail AS B', array('A.deleted'=>0,'B.deleted'=>0,'A.category_id'=>$category_id));
        $data = $query->result_array();
        return $data;
    
    }
    
    public function get_items_by_supplier($supplier_id)
    {
        //$this->db->order_by('A.item_id','desc');
        $this->db->select('A.item_id,A.service,A.category_id, A.name,
        B.color_id,B.quantity,B.cost_price, B.avg_cost,B.unit_price');
        $this->db->join('pos_items_detail AS B','A.item_id = B.item_id');
        //$this->db->join('pos_taxes AS T','T.id = B.tax_id','left');
        //$this->db->join('pos_sizes as C','B.size_id = C.id','left');
        
        $query = $this->db->get_where('pos_items AS A', array('A.deleted'=>0,'B.deleted'=>0,'B.supplier_id'=>$supplier_id));
        $data = $query->result_array();
        return $data;
    ...
    function addItems()
        {
            ...
            $landed_cost = ($cost_price+$this->input->post('dtl_cost',true));
            $local_currency = ($landed_cost*$this->input->post('php_cost',true));
            $shipping_terms = $this->input->post('shipping_term_id',true);
     ...
     function updateItems($new_picture = null)
     {
            ...
            $service = $this->input->post('service',true);
            $cost_price = $this->input->post('avg_cost',true);
            $landed_cost = ($cost_price+$this->input->post('dtl_cost',true));
            $local_currency = ($landed_cost*$this->input->post('php_cost',true));
            ...
            }else
            {
                ...
                'unit_price' =>$this->input->post('unit_price',true),
                'quoted_qty'=>$this->input->post('quoted_qty',true),
                'tax_id' => $this->input->post('tax_id',true),
                'avg_cost' =>$cost_price,
                'cost_price' =>$cost_price,
                'location_id' =>$this->input->post('location_id',true),
    ...
    function get_item_history($item_id,$from_date,$to_date)
    {
