...
class m_estimate extends CI_Model{
    ...
    function get_estimate($estimate_id = FALSE, $from_date = null, $to_date=null)
    {
       ...
       $query = $this->db->get_where('pos_estimate',array('sale_id'=>$sale_id,'company_id'=> $_SESSION['company_id']));
       return $query->result_array();
    }
    ...
    function get_selected_estimate($from_date = null, $to_date=null)
    {
        ...
        s.customer_id,s.employee_id,e.first_name as emp,c.store_name as customer');
...
//    }
    ...
    function get_estimate_items($new_invoice_no)//for receipt
    {
       $this->db->select('A.sale_date,A.amount_due,A.register_mode,A.employee_id,A.discount_value as total_discount,A.customer_id,
       A.currency_id,A.description,A.invoice_no,A.account,
