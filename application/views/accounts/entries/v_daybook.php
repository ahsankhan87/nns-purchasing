...
<div class="row">
    <div class="col-sm-12">
       <?php
        ...
        <table class="table table-striped table-condensed flip-content" id="">
        ...
        $.ajax({
                ...
                success: function(data) 
                {
                    ...
                    $.each(data,function(index, value){
                        ...
                        view_journal +=     '<td>'+value.title+'</td>'+
                                            '<td style="text-align: right;">'+(value.debit == '0' ? '' : parseFloat(value.debit).toFixed(2))+'</td>'+
                                            '<td style="text-align: right;">'+(value.credit == '0' ? '' : parseFloat(value.credit).toFixed(2))+'</td>'+
                                            '<td>'+value.narration+'</td>';

                                            
                                view_journal += '<td><a href="'+site_url+'/accounts/C_entries/delete/'+value.id+'/'+value.entry_id+'" title="Delete" onclick="return confirm(\'Are you sure you want to permanent delete? All entries associated with this entry will be deleted.\')"><i class="fa fa-trash-o fa-fw"></i></a></td>';
                                view_journal +=  '</tr>';
