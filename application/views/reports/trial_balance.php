...
<div class="row">
    ...
    foreach($trialBalance as $key => $list)
    {
        ...
        //{
            ...
            if($balance > 0){
                echo '<td>'.round(abs($balance),2).'</td>';
                echo '<td>0.00</td>';
                $dr_amount += $balance;
            }
            elseif($balance < 0){
    ...
    echo '<td>'.'<small>'.$_SESSION['home_currency_symbol'].'</small>'.'<strong>'.round(abs($dr_amount),2).'</strong></td>';
    echo '<td>'.'<small>'.$_SESSION['home_currency_symbol'].'</small>'.'<strong>'.round(abs($cr_amount),2).'</strong></td>';
