...
class C_suppliers extends MY_Controller{
    ...
    function ProductsBySupplier($supplier_id)
    {
        $data = array('langs' => $this->session->userdata('lang'));
        ...
        //get all categories.
        $data['products']= $this->M_items->get_items_by_supplier($supplier_id);
        
        //$data['check_cate'] = $this->M_category->get_category(16);
        
        $this->load->view('templates/header',$data);
        $this->load->view('pos/suppliers/v_products',$data);
        $this->load->view('templates/footer');
    }
    ...
    public function get_supplier_dtl_cost($supplier_id)
    {
        echo json_encode($this->M_suppliers->get_supplier_dtlcost($supplier_id));
...
//            endforeach;
           ...
           if($discount_amount > 0)
           {
                if(@$_SESSION['multi_currency'] == 1)
                {
                   ...
                   $dr_account = $posting_type_code[0]['payable_acc_code'];//supplier ledger id
                   $entry_id=$this->M_entries->addEntries($dr_account,$cr_account,$discount_amount/$exchange_rate,$discount_amount/$exchange_rate,$narration,$new_invoice_no,$date);
    ...
    function create()
    {
        ...
        if($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if($this->form_validation->run())
            {
                ...
                "dtl_cost" => $this->input->post('dtl_cost', true),
                "php_cost" => $this->input->post('php_cost', true)
                );
